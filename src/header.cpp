...
void parse_rest_elememt_of_sliceHeader(bs_t *b) {
    ...
    if (slice_header->slice_type == Slice_Type_P || slice_header->slice_type == Slice_Type_SP || slice_header->slice_type == Slice_Type_B) {
    ...
    if (active_pps->entropy_coding_mode_flag && slice_header->slice_type != Slice_Type_I && slice_header->slice_type != Slice_Type_SI) {
    ...
    if (slice_header->slice_type == Slice_Type_SP || slice_header->slice_type == Slice_Type_SI) {
...
void parse_ref_pic_list_modification(bs_t *b) {
    ...
    if (currentSlice->slice_header.slice_type != Slice_Type_I && currentSlice->slice_header.slice_type != Slice_Type_SI) {
...
void alloc_ref_pic_list_modification_buffer() {
    ...
    if (currentSlice->slice_header.slice_type != Slice_Type_I && currentSlice->slice_header.slice_type != Slice_Type_SI) {
        ...
        if (rplm->modification_of_pic_nums_idc_lo == NULL || rplm->abs_diff_pic_num_minus1_lo == NULL || rplm->long_term_pic_num_lo == NULL) {
    ...
    if (currentSlice->slice_header.slice_type == Slice_Type_B) {
        ...
        if (rplm->modification_of_pic_nums_idc_l1 == NULL || rplm->abs_diff_pic_num_minus1_l1 == NULL || rplm->long_term_pic_num_l1 == NULL) {
