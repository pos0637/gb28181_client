...
/**
 ...
 @return nalu的大小
 */
int find_nal_unit(uint8_t *file_buff, nalu_t *nalu, int buff_size, int *curr_nal_start, int *curr_find_index) {
    int *i = curr_find_index;

    //( next_bits( 24 ) != 0x000001 && next_bits( 32 ) != 0x00000001 )
    // 寻找起始码，只要有一位不满足，则继续向下寻找
    while ((file_buff[*i] != 0x00 || file_buff[*i + 1] != 0x00 || file_buff[*i + 2] != 0x01) &&
    ...
    *curr_nal_start = *i;
    ...
    while ((file_buff[*i] != 0x00 || file_buff[*i + 1] != 0x00 || file_buff[*i + 2] != 0x00) && (file_buff[*i] != 0x00 || file_buff[*i + 1] != 0x00 || file_buff[*i + 2] != 0x01)) {
...
int rbsp_to_sodb(nalu_t *nalu) {
    ...
    while (ctr_bit == 0) {
        ...
        if (bitoffset == 8) {
            ...
            if (last_byte_pos == 1) printf(" Panic: All zero data sequence in RBSP \n");
...
void freeNalu(nalu_t *nalu) { free(nalu); }
